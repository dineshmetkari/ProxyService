package com.bay.webservicex.utility.parser.soaptojson;

import com.bay.webservicex.utility.parser.soaptojson.helpers.XmlNodeToJsonElement;
...
public class Parser {
     ...
     * @return XML document
     */
    public static Document parse(JsonReader reader) throws IOException, Parsefailure {
        reader.beginArray();
    ...
    protected static void addInitialElement(JsonReader reader, CoreDocumentImpl doc) throws IOException, Parsefailure {
    ...
    protected static void addInitialDocNode(CoreDocumentImpl doc, JsonReader reader) throws IOException, Parsefailure {
    ...
    public static Node parseNextNode(CoreDocumentImpl doc, JsonReader reader) throws IOException, Parsefailure {
    ...
    protected static void setChildren(CoreDocumentImpl doc, JsonReader reader, Element e) throws IOException, Parsefailure {
        //Children
        reader.beginArray();
        while (!reader.peek().equals(JsonToken.END_ARRAY)) {
    ...
    protected Object[] getChildren(Node n) throws Parsefailure {
        switch (n.getNodeType()) {
            ...
            default:
                throw new Parsefailure(n);
        }
    }

    protected List<Object> getConvertedChildNodes(Node n) throws Parsefailure {
    ...
    public String stringify(Node n) throws Parsefailure {
     ...
     * @return XML document
     */
    public Document toXml(InputStream json) throws IOException, Parsefailure {
    ...
    public Document toXml(String json) throws IOException, Parsefailure {
        InputStream is = new ByteArrayInputStream(json.getBytes(StandardCharsets.UTF_8));
        return toXml(is);
    }

    public Object fromXml(Node n) throws Parsefailure {
        return convert(n);
    }

    public Object convert(Node n) throws Parsefailure {
        switch (n.getNodeType()) {
            case Node.CDATA_SECTION_NODE:
            case Node.COMMENT_NODE:
                // [nodeType, nodeVal]
    ...
    protected String[][] getAttributes(Node n) throws Parsefailure {
    ...
    protected String[][] convertNamedNodeMap(NamedNodeMap attrs) throws Parsefailure {
        String[][] attributes = new String[attrs.getLength()][];
        for (int i = 0; i < attrs.getLength(); i++) {
            attributes[i] = (String[]) convert(attrs.item(i));
        }
